...
const Index = () => {
  ...
  return (
    <div className="flex flex-col items-start w-full p-8">
        ...
        {Object.keys(components).map((tabName, index) => (
          <React.Fragment key={tabName}>
            <button
              ...
              className={`flex items-center justify-center w-1/6 px-6 py-2 rounded-full border ${getButtonClass(
                tabName
              )}`}
            >
      ...
      <div className="w-full flex bg-white  items-center rounded-3xl justify-center">
        {renderComponent()}
      </div>
    </div>
  );
